...
namespace FacePost.Adapter
{
    class PostAdapter : RecyclerView.Adapter
    {
        ...
        //This method finds the images through URL and inserts them in a row
        //This is because we dont want the images to be saved permanently on a users phone
        void GetImage(string url, ImageView imageView)
        {
            ImageService.Instance.LoadUrl(url)
                .Retry(3, 200) //Will try download an image 3 times with a delay on 200 ms
                .DownSample(400, 400) //Compressing Image - Size is 400 x 400 to reduce image size
                .Into(imageView); //Sends the image to an ImageView
        }
